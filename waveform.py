...
the l.h.s of the system of ODE equations
    ...
    M    = M0*Ms              # Restore units in central mass
    μ    = μ0*Ms              # Restore units in orbiting mass      
    spin = spin0*(G/c)        # Restore units in spin
    
    x    = (2*Pi*G*M*ν)/c**3  # Post-Newtonian parameter
    
    ###### System of ODEs for {Φ, ν, e, γ, α} ######
    dPhi_dt = 2*Pi*ν
    #
    dnu_dt = (96/(10*Pi))*((c**6/G**2)*μ/M**3)*(x**(11/3))*(1 - e**2)**(-9/2)*((1 - e**2)*(1 + (73/24)*e**2 + (37/96)*e**4) + x**(2/3)*((1273/336) - (2561/224)*e**2 - (3885/128)*e**4 - (13147/5376)*e**6) - x*(c/G*spin)*np.cos(λ)*(1 - e**2)**(-1/2)*((73/12) + (1211/24)*e**2 + (3143/96)*e**4 + (65/64)*e**6)) 
    #
    dgamma_dt = 6*Pi*ν*x**(2/3)*((1 - e**2)**(-1))*(1 - 2*(x**(1/3))*(c/G*spin)*np.cos(λ)*(1 - e**2)**(-1/2)+ (1/4)*x**(2/3)*(1 - e**2)**(-1)*(26 - 15*e**2))
    #
    de_dt = -(e/15)*((c**3/G)*μ/M**2)*(x**(8/3))*((1 - e**2)**(-7/2))*((1 - e**2)*(304 + 121*e**2)+ (1/7)*(x**(2/3))*(8831 - 28995*e**2 - (56101/8)*e**4) - 3*x*(c/G*spin)*np.cos(λ)*((1-e**2)**(-1/2))*(1364 + (5032/3)*e**2 + (263/2)*e**4)) 
    #                       
    dalpha_dt = 4*Pi*ν*x*(c/G*spin)*(1 - e**2)**(-3/2)
    #############################
...
################## compute_orbit()
def compute_orbit(p, x, t_min = t_min, t_max = t_max, solver = solver0):
    """
...
solution = [Φ, ν, e, γ, α], where each component of array is a 1D vector of real values 
    ...
    # call the ODE solver 
    sol0   = solve_ivp(eqs, (t_max, t_min), x0, method=solver0, t_eval = t_span, args=args, rtol =rtol, atol=atol)
    if sol0.success==True: # check that the solution is succesful 
...
-------
    ...
    θS, φS, θK, φK, φ0, T1year, R = Pi/4, 0, Pi/8, 0, 0, 3.127*10**7, 499 
    
    x = cp.asarray((2*Pi*G*M*ν)/c**3,dtype=np.float64)
    A = cp.asarray(x**(2/3)*(G*μ/c**2),dtype=np.float64)
    ...
    θ = cp.arccos(cp.cos(θS)/2 - (cp.sqrt(3)*cp.sin(θS)/2*cp.cos((2*Pi*t)/T1year - φS))) 
    φ = (2*Pi*t)/T1year + cp.arctan2((cp.sqrt(3)*cp.cos(θS) + cp.sin(θS)*cp.cos((2*Pi*t)/T1year - φS)), (2*(cp.sin(θS)*(cp.sin((2*Pi*t)/T1year - φS)))))
    ψ = cp.arctan2((cp.cos(θL)/2 - (cp.sqrt(3)*cp.sin(θL)*cp.cos((2*Pi*t)/T1year - φL))/2 - cp.cos(θ)*(cp.cos(θS)*cp.cos(θL) + cp.cos(φL - φS)*cp.sin(θS)*cp.sin(θL))), (0.5*(cp.sin(θL)*cp.sin(θS)*cp.sin(φL - φS)) - 0.5*(cp.sqrt(3)*cp.cos((2*Pi*t)/T1year)*(cp.cos(θL)*cp.sin(θS)*cp.sin(φS) - cp.cos(θS)*cp.sin(θL)*cp.sin(φL))) - 0.5*(cp.sqrt(3)*cp.sin((2*Pi*t)/T1year)*(cp.cos(θS)*cp.cos(φL)*cp.sin(θL) - cp.cos(φS)*cp.cos(θL)*cp.sin(θS)))))
...
# (2) For n=-1 (the first iteration of loop below) - the function result is not defined for the cuda implementation so we have to use sci-py implementation - b0:
    ...
    for n in range(1,nmax+1): # sums over orbital overtones n. Integer nmax is the maximum overtone to sum.
        ...
        if (n==1): jn_nm2_nz = b0 #fix the special case where cuda function is not giving what we need. See above.
...
2D array, [ [time grid, amplitude of fiducial waveform], [frequency grid, FFT of amplitude of fiducial waveform] ] 
    ...
    WF0      = waveform(n_max, M0, μ0, spin0, sol0[0], sol0[1]) # compute waveform for fiducial model
...
plot of the waveform in the sence y = y(x). The plotted waveform is not rescaled by the lum. distance ~ 1/D.
    ...
    plt.xlim(t_max - 60*60*hours, t_max)
    plt.xlabel('time (sec)')
